...
import { UserWithToken } from "src/api/auth.js";
import { UnauthorizedError } from "../../api/errors.js";
import { db } from "../index.js";
import { users, User, refresh_tokens } from "../schema.js";
...
export async function addRefreshToken(userData: UserWithToken) {
...
export async function getUserFromRefreshToken(currentToken: string){
    const [result] = await db
    .select({userId: refresh_tokens.userId}) //selects just the UUID for this token if its valid
    .from(refresh_tokens)
    .where(and(
        eq(refresh_tokens.token, currentToken),
...
export async function updateRefreshToken(userId: string, newToken: string) {
    await db
...
export async function revokeRefreshToken(currentToken: string){
    await db
    .update(refresh_tokens)
    .set({
        updatedAt: new Date(),
